...
1. **开启机场**：选新加坡等节点，建议 TUN / 增强模式，确保币安 API 可调用
2. **启动 MCP**：在「加密货币 MCP 服务器」仓库执行 `python3 unified_server.py`（默认 `8080`），用 `curl http://127.0.0.1:8080/health` 确认
3. **启动 AI 助手**：本仓库执行 `npm start` 或 `npm run dev`，浏览器打开 `http://localhost:3000`

更完整的说明见 [README.md · 本地使用](README.md#本地使用)。
